...
class InstrumentWindow(BaseWindow):
    def __init__(self, ui: UserInterface, source: str, instrument_info: typing.Dict[str, typing.Any]):
        ...
        self._active_setpoint: typing.Optional[typing.Dict[str, typing.Any]] = None
        self._next_setpoint: typing.Optional[typing.Dict[str, typing.Any]] = None

    def instrument_message(self, record: str, message: typing.Any) -> bool:
        if record == 'active':
            self._active_setpoint = message
            return True
        elif record == 'next':
            self._next_setpoint = message
            return True
    ...
    def data_lines(self) -> typing.List[typing.Tuple[str, str]]:
        ...
        if self._active_setpoint:
            setpoint = self._active_setpoint.get('setpoint')
            t = self._active_setpoint.get('epoch_ms')
            if t:
                t = format_time_of_day(t / 1000.0)
            else:
                t = ""
            result.append((f"Current setpoint: {setpoint:.1f}%", t))

        if self._next_setpoint:
            setpoint = self._next_setpoint.get('setpoint')
            t = self._next_setpoint.get('epoch_ms')
            if t:
                t = self._countdown_time(t / 1000.0)
            else:
                t = ""
            result.append((f"Next setpoint: {setpoint:.1f}%", t))
