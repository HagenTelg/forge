        ...
        while True:
            ...
            try:
                if line == b'DC':
                    ...
                    self.writer.write(f"{self.data_Toptics:.1f}\r".encode('ascii'))
                elif line == b'R4':
                    self.writer.write(f"{flow_lpm_to_ccs(self.data_Q):.2f}\r".encode('ascii'))
                elif line == b'R5':
                    self.writer.write(b"READY\r")
                elif line == b'X5':
                    self.writer.write(b"OK\r")
                elif line == b'I':
                    self.writer.write(b"Calibration Label\r")
                elif line == b'C':
                    self.writer.write(b"0,0\r")
                else:
                    raise ValueError
