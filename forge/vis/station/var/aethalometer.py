...
class AE36(TimeSeries):
    ...
    def __init__(self, mode: str, **kwargs):
        ...
        factor = self.SevenWavelength(None, '.6f', "Correction {wavelength} nm ({size})",
                                      f'{mode}-aethalometer', 'CF{index}')
        factor.title = "Loading Correction Factor"
        self.graphs.append(factor)


class EditingAE36(TimeSeries):
    SevenWavelength = AE36.SevenWavelength
...
class AE33Status(TimeSeries):
    ...
    def __init__(self, mode: str, **kwargs):
        super().__init__(**kwargs)
        self.title = "AE36 Status"

        transmittance = self.SevenWavelength(None, '.7f', "Transmittance {wavelength} nm ({size})",
...
class AE36OpticalStatus(TimeSeries):
    def __init__(self, mode: str, **kwargs):
        super().__init__(**kwargs)
        self.title = "AE36 Status"
        ...
        ebc = AE36.SevenWavelength("μg/m³", '.3f', "X {wavelength} nm ({size})", f'{mode}-aethalometer', 'X{index}')
        ebc.title = "Equivalent Black Carbon"
        ebc.contamination = f'{mode}-contamination'
        self.graphs.append(ebc)
