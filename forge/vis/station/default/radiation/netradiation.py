...
function net(direct, diffuse, up_sw, down_lw, up_lw, zsa) {
    ...
    if (diffuse > 1500 * 0.95 * u0 ** 1.2 + 50) {
        return undefined;
    }
    const down_sw = direct * u0 + diffuse;
    return (down_sw - up_sw) + (down_lw - up_lw);
}
function netshortwave(direct, diffuse, up_sw, down_lw, up_lw, zsa) {
...
function netlongwave(direct, diffuse, up_sw, down_lw, up_lw, zsa) {
    if (!isFinite(down_lw) || !isFinite(up_lw)) {
        return undefined;
    }
    return down_lw - up_lw;
}
    return new GenericOperations.ApplyToFields(dataName, { net: net, netshortwave: netshortwave, netlongwave: netlongwave }, 'direct', 'diffuse', 'up_sw', 'down_lw', 'up_lw', 'zsa');
})"""
