...
class Parameters(RecordConverter):
    ...
    def metadata(self) -> cpd3_variant.Metadata:
        ...
        meta.children["CalibrationSlope"] = cpd3_variant.MetadataArray({
            "Description": "Raw calibration slopes",
            "Children": cpd3_variant.MetadataReal({"Description": "Angle calibration slope"})
        })
        meta.children["CalibrationOffset"] = cpd3_variant.MetadataArray({
            "Description": "Raw calibration offsets",
            "Children": cpd3_variant.MetadataReal({"Description": "Angle calibration offset"})
        })
        return meta
    ...
    def value(self) -> typing.Dict[str, typing.Any]:
        ...
        raw_data = self.group.variables.get("calibration_slope")
        if raw_data is not None:
            result["CalibrationSlope"] = raw_data[:].data.tolist()
        raw_data = self.group.variables.get("calibration_offset")
        if raw_data is not None:
            result["CalibrationOffset"] = raw_data[:].data.tolist()
