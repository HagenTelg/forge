...
def default_converter(source: "RawInstrument", station: str, instrument_id: str,
                      file_start: float, file_end: float) -> typing.Optional[typing.Type["InstrumentConverter"]]:
    ...
    if source.source.cpd3_component == "acquire_aerodyne_caps":
        ...
        return Converter
    if source.source.cpd3_component == "acquire_dmt_pax":
        from .dmtpax import Converter
        return Converter
    ...
    if source.source.cpd3_component == "acquire_magee_tca08":
        ...
        return Converter
    if source.source.cpd3_component == "acquire_dmt_bcp":
        from .dmtbcp import Converter
        return Converter
    if source.source.cpd3_component == "acquire_azonix_umac1050" or source.source.forge_instrument == "azonixumac1050":
        ...
        return Converter.with_variables(cut_split, unsplit)
...
def convert_raw(source: "RawInstrument", station: str, instrument_id: str,
    ...
    c = default_converter(source, station, instrument_id, file_start, file_end)
    if c is not None:
        return c(station, instrument_id, file_start, file_end, root).run()
    return False


def convert_clean(source: "RawInstrument", station: str, instrument_id: str,
                  file_start: float, file_end: float, root: Dataset) -> bool:
    c = default_converter(source, station, instrument_id, file_start, file_end)
    if c is not None:
        class AlternateConverter(c):
            @property
            def archive(self) -> str:
                return "clean"
        return AlternateConverter(station, instrument_id, file_start, file_end, root).run()
    if instrument_id == "XI" or instrument_id == "XJ" or instrument_id == "XK":
        from .intensives import Converter
