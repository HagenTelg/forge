...
class Converter(InstrumentConverter):
    ...
    def tags(self) -> typing.Optional[typing.Set[str]]:
        return {"aerosol", "met", "gillwindsonic"}

    @property
    def instrument_type(self) -> typing.Optional[str]:
        return "gillwindsonic"

    @property
    def average_interval(self) -> typing.Optional[float]:
        return self._average_interval

    def _metadata_parameters(self, meta) -> typing.Dict[str, typing.Tuple[str, str]]:
        unit_configuration = meta.get("UnitConfiguration")
        if not unit_configuration:
            return dict()

        g = self.root.groups.get("parameters")
        if g is None:
            ...
            var_parameters.long_name = "unit configuration response to the D3 command"
        else:
            var_parameters = g.variables["instrument_parameters"]

        var_parameters[0] = str(unit_configuration)
        return dict()
    ...
    def run(self) -> bool:
        ...
        self.apply_instrument_metadata(f"WS_{self.instrument_id}", manufacturer="Gill Instruments", model="Windsonic", extra=self._metadata_parameters)
