...
class Met(InstrumentConverter):
    ...
    def run(self) -> bool:
        ...
        if data_X1.time.shape[0] > 0:
            var_X1 = g.createVariable("mass_concentration", "f8", ("time",), fill_value=nan)
            ...
            var_X1.long_name = "mass concentration of PM2.5 particles"
            var_X1.units = "ug m-3"
            var_X1.C_format = "%7.2f"
            self.apply_data(times, var_X1, data_X1)
            
        if data_X2.time.shape[0] > 0:
            var_X2 = g.createVariable("ozone_mixing_ratio", "f8", ("time",), fill_value=nan)
            netcdf_var.variable_ozone(var_X2)
        ...
        if data_X3.time.shape[0] > 0:
            var_X3 = g.createVariable("nitrogen_monoxide_mixing_ratio", "f8", ("time",), fill_value=nan)
            netcdf_var.variable_no(var_X3)
            netcdf_timeseries.variable_coordinates(g, var_X3)
            var_X3.variable_id = "X2"
        ...
        if data_X5.time.shape[0] > 0:
            var_X5 = g.createVariable("sulfur_dioxide_mixing_ratio", "f8", ("time",), fill_value=nan)
            ...
            var_X5.long_name = "fraction concentration of sulfur dioxide"
            var_X5.standard_name = "mole_fraction_of_sulfur_dioxide_in_air"
            var_X5.units = "1e-9"  # canonical ppb
            var_X5.C_format = "%9.2f"
...
C.run(STATION, {
    "A11": [ C('clap', start='2018-07-17'), ],
    "N61": [ C('tsi377xcpc', start='2018-07-16'), ],
    "S11": [ C('tsi3563nephelometer', start='2018-07-16'), ],
    "XM1": [ C(Met, end='2020-01-02'), ],
})
