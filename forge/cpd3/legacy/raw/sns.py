...
from forge.cpd3.legacy.instrument.dmtccn import Converter as CCN
...
class CCN200Secondary(InstrumentConverter):
    ...
    @property
    def tags(self) -> typing.Optional[typing.Set[str]]:
        return {"aerosol", "dmtccn200", "secondary"}
    ...
    def run(self) -> bool:
        data_N = self.load_variable(f"Ns_N21")
        ...
        data_ZBin = self.load_variable(f"ZBins_N21", dtype=np.int64)
        data_Nb = self.load_array_variable(f"Nbs_N21")
        data_Np = self.load_array_variable(f"Nps_N21")
        data_Q1 = self.load_variable(f"Q1s_N21")
        data_Q2 = self.load_variable(f"Q2s_N21")
        data_P = self.load_variable(f"Ps_N21")
        data_U = self.load_variable(f"Us_N21")
        data_Tu = self.load_variable(f"Tus_N21")
        data_T1 = self.load_variable(f"T1s_N21")
        data_T2 = self.load_variable(f"T2s_N21")
        data_T3 = self.load_variable(f"T3s_N21")
        data_T4 = self.load_variable(f"T4s_N21")
        data_T5 = self.load_variable(f"T5s_N21")
        data_T6 = self.load_variable(f"T6s_N21")
        data_DT = self.load_variable(f"DTs_N21")
        data_DTg = self.load_variable(f"DTgs_N21")
        data_V1 = self.load_variable(f"V1s_N21")
        data_V2 = self.load_variable(f"V2s_N21")
        data_A = self.load_variable(f"As_N21")
        data_Uc = self.load_variable(f"Ucs_N21")
        ...
        self.apply_coverage(g, times, f"Ns_N21")

        self.apply_instrument_metadata(f"Ns_N21", manufacturer="DMT", model="CCN200")
...
C.run(STATION, {
    "A21": [
        C('thermomaap', end='2019-09-12'),
        C('thermomaap', start='2024-04-26'),
    ],
    "A41": [ C('mageeae33', start='2019-05-30', end='2023-07-28'), ],
    "A42": [
        C('mageeae33+secondary', start='2019-05-30', end='2023-07-28'),
        C('mageeae33', start='2023-07-28', end='2024-12-19'),
    ],
    "A61": [ C('mageeae31+secondary', start='2018-05-23', end='2018-07-18'), ],
    "A71": [ C('mageeae31+secondary', start='2018-05-23', end='2018-07-03'), ],
    "A81": [
        C('mageeae33+secondary', end='2017-05-12'),
        C('mageeae31+secondary', start='2017-05-12', end='2017-06-14'),
        C('mageeae33+secondary', start='2017-06-14', end='2019-05-31'),
    ],
    "N21": [
        C("dmtccn", start='2019-05-30', end='2021-07-14'),
        C(CCN200, start='2021-07-14', end='2022-07-07'),
    ],
    "N22": [ C(CCN200Secondary, start='2021-07-14', end='2022-07-07'), ],
    "S11": [ C('tsi3563nephelometer'), ],
    "S12": [ C('tsi3563nephelometer+secondary', end='2017-03-07'), ],
    ...
    }), start='2016-05-10', end='2017-10-05'), ],
})
